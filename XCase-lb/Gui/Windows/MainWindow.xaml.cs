...
namespace XCase.Gui
{
    ...
    public partial class MainWindow
    {
        ...
        {
            foreach (DocumentContent document in dockManager.Documents)
            {
                document.Close();
            }

            foreach (DockableFloatingWindow floatingWindow in dockManager.FloatingWindows)
            {
                floatingWindow.Close(true);
            }

            while (DiagramTabManager.CreatedFloatingWindows.Count > 0)
            {
                DiagramTabManager.CreatedFloatingWindows.First().Close();
            }

            DiagramTabManager.CreatedFloatingWindows.Clear();
        ...
        {
            cmdOpenProject open = new cmdOpenProject(this, null) { NoDialogs = true, Filename = filename };
            open.Execute();
        ...
        private static string LayoutSettingsFile
        {
            get
            {
                return Path.Combine(Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().Location), "layout.xml");
            }
        }
        ...
        private void test_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (file != null)
            {
                try
                {
                    ...
                    c.Execute();
                    //todo: File.WriteAllText(TEST_OUTPUT_PATH + Path.GetFileNameWithoutExtension(file) + ".xsd", c.Window.XMLSchemaText,
